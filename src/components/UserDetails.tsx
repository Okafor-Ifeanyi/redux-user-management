...
//   const queryClient = useQueryClient();
  ...
  const [user, setUser] = useState({ id: id ? Number(id) : users.length + 1, name: "", email: "", address: { street: "", city: "" } });
            ...
            <form onSubmit={handleSubmit} className="addUserForm">
                ...
                <input type="text" placeholder="Name"
                    value={user.name} onChange={(e) => setUser({ 
                        ...user, name: e.target.value })} readOnly 
                />
                ...
                <div className="detail-buttons">
                    ...
                    <button type="button" onClick={() => navigate(`/edit-user/${user.id}`)}>edit user</button>
                </div>
            </form>
        </FormBackground>
    </>
